...
const CardHoverRevealMain = React.forwardRef(({ className, initialScale = 1, hoverScale = 1.05, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn("size-full transition-transform duration-500 ease-out", className)}
      style={
        isHovered
...
const CardHoverRevealContent = React.forwardRef(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "absolute inset-[auto_1rem_1rem] sm:inset-[auto_1.5rem_1.5rem] p-6 backdrop-blur-xl transition-all duration-500 ease-in-out z-20",
        className
      )}
      style={
        isHovered
